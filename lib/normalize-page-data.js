...
function normalizeNavigation(source, pageType) {
  ...
  const localLinks = (navigation.local_links || 
    [
      ...
      { label: 'Planner', url: pageType === 'guide' ? '#plan-your-trip' : `${homeUrl}#plan-your-trip`, key: 'planner' }
    ]).map((link) => ({
      ...link,
      url:
...
function normalizeNeighbourhoodMap(items, citySlug) {
  ...
  return items
    .map((item, index) => {
      ...
      const coordinates = cityCoordinates[slug] || fallbackPoints[index % fallbackPoints.length];
...
const NEIGHBOURHOOD_MAP_COORDINATES = {
  amsterdam: {
    noord: { x: 54, y: 16, tone: 'tone-3' },
    westerpark: { x: 28, y: 32, tone: 'tone-2' },
    jordaan: { x: 36, y: 43, tone: 'tone-1' },
    wallen: { x: 51, y: 40, tone: 'tone-4' },
    centrum: { x: 48, y: 50, tone: 'tone-1' },
    plantage: { x: 65, y: 51, tone: 'tone-2' },
    'oud-west': { x: 31, y: 61, tone: 'tone-3' },
    museumplein: { x: 43, y: 70, tone: 'tone-1' },
    'de-pijp': { x: 55, y: 73, tone: 'tone-4' },
    oost: { x: 74, y: 66, tone: 'tone-2' }
  }
};
